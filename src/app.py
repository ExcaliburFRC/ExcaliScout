...
def manual_assign():
    data = request.json
    assignments = data['matches']

    # Debugging: Print the incoming data
    print("Incoming data:", assignments)
    ...
    for match in assignments:
        ...
        required_keys = ['scouter1', 'scouter2', 'scouter3', 'scouter4', 'scouter5', 'scouter6', 'match_id']
        for key in required_keys:
            if key not in match:
                return jsonify({'status': 'failure', 'message': f'Missing key: {key}'}), 400
